...
public final class SprayBrush implements BetterBrush {
    ...
    public SprayBrush(Player player, BrushSettings brushSettings, Plugin plugin) {
        this.player = player;
        ...
        if (itemStack.getItemMeta() instanceof SkullMeta skullMeta) {
            ...
            profile.setProperty(new ProfileProperty("textures", Constants.HEAD_SPRAY));
        ...
        for (double x = positionA.getX(); x <= positionB.getX(); x++) {
            for (double z = positionA.getZ(); z <= positionB.getZ(); z++) {
                for (double y = positionA.getY(); y <= positionB.getY(); y++) {
                    ...
                    if (random.nextInt(100)< settings().chance) {
