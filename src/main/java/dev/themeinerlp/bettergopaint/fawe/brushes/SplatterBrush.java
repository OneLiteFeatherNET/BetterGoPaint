...
public final class SplatterBrush implements BetterBrush {
    ...
    public SplatterBrush(Player player, BrushSettings brushSettings, Plugin plugin) {
        this.player = player;
        ...
        if (itemStack.getItemMeta() instanceof SkullMeta skullMeta) {
            ...
            profile.setProperty(new ProfileProperty("textures", Constants.HEAD_SPLATTER));
        ...
        for (double x = positionA.getX(); x <= positionB.getX(); x++) {
            for (double z = positionA.getZ(); z <= positionB.getZ(); z++) {
                for (double y = positionA.getY(); y <= positionB.getY(); y++) {
                    ...
                    double rate = (blockInRadius
                            .distance(playerPosAsVec3.toBlockPoint()) - (size / 2.0) * ((100.0 - (double) settings().falloffStrength) / 100.0)) / ((size / 2.0) - (size / 2.0) * ((100.0 - (double) settings().falloffStrength) / 100.0));
